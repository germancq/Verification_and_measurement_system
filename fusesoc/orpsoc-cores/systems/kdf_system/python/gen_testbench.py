...
def gen_all_posibilities(micro_sd):
    ...
    parameters = [
        np.random.randint(0,2**31-1,5), #salt values
        [50,100,500], #count values
        np.random.randint(0,2**31-1,5), #password values
    ]
    ...
    for n in range (0, total_posibilities):
        ...
        micro_sd.write(int(pairs[0]).to_bytes(8, byteorder='big'))#salt
        micro_sd.write(int(pairs[1]).to_bytes(4, byteorder='big'))#count
        micro_sd.write(int(pairs[2]).to_bytes(4, byteorder='big'))#password
