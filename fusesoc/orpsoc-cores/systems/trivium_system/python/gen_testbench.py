...
def gen_all_posibilities(micro_sd):
    ...
    parameters = [
        np.random.randint(0,2**31-1,8), #input_1 values
        np.random.randint(0,2**31-1,8), #input_2 values
    ]
    ...
    for n in range (0, total_posibilities):
        ...
        micro_sd.write(int(pairs[0]).to_bytes(10, byteorder='big'))#iv
        micro_sd.write(int(pairs[1]).to_bytes(10, byteorder='big'))#key
        

        micro_sd.write(zero.to_bytes(8, byteorder='big'))
